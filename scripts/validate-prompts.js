...
const VALID_CATEGORIES = ['AI绘画', '写作', '编程', '效率提升', '翻译', '教育', '营销', '分析']
  ...
  data.prompts.forEach((prompt, index) => {
    ...
    if (!prompt.id) {
      ...
      if (!/^[a-z-]+-\d{3}$/.test(prompt.id)) {
        warnings.push(
          `[${promptId}] ID doesn't follow recommended format: {category-slug}-{number}`,
        )
      }
    }
    ...
    if (!Array.isArray(prompt.tags)) {
      ...
      warnings.push(
        `[${promptId}] Too many tags (${prompt.tags.length}), consider reducing to 8 or fewer`,
      )
    }
    ...
    if (prompt.status) {
      ...
      if (!allowed.includes(prompt.status)) {
        warnings.push(
          `[${promptId}] Unknown status: ${prompt.status} (allowed: ${allowed.join(', ')})`,
        )
      }
    }
  })
