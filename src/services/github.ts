...
async function fetchJson<T>(url: string, init?: RequestInit, retries = 2): Promise<{ ok: boolean; status: number; data: T }> {
...
export async function getDefaultBranch(owner: string, repo: string, token: string): Promise<string> {
  const { ok, data } = await fetchJson<{ default_branch?: string }>(`${API}/repos/${owner}/${repo}`, {
...
export async function getBranchSha(owner: string, repo: string, branch: string, token: string): Promise<string> {
  const { ok, data } = await fetchJson<any>(`${API}/repos/${owner}/${repo}/git/refs/heads/${encodeURIComponent(branch)}`, {
    headers: baseHeaders(token),
  })
...
export async function getFile(owner: string, repo: string, path: string, ref: string, token: string): Promise<{ sha: string; content: string }> {
  const { ok, data } = await fetchJson<any>(`${API}/repos/${owner}/${repo}/contents/${encodeURIComponent(path)}?ref=${encodeURIComponent(ref)}`, {
    headers: baseHeaders(token),
  })
  const sha = data?.sha
...
export async function updateFile(owner: string, repo: string, path: string, newContent: string, message: string, branch: string, sha: string, token: string) {
  ...
  const { ok } = await fetchJson(`${API}/repos/${owner}/${repo}/contents/${encodeURIComponent(path)}`, {
    method: 'PUT',
    headers: { ...baseHeaders(token), 'Content-Type': 'application/json' },
